...
contract TRDCvault is BEP20 {
    ...
    uint112 private currencyReserve;
    uint32 private blockTimeStampLast;
    address public dEaD= 0x000000000000000000000000000000000000dEaD;
    uint fractions = 10** 18;
    uint fractionsT;
    uint public cardPrice = 100 * fractions;
    uint smallPortion;
    uint cardPower;
    uint randomThief;
    uint randomCop;
    ...
    uint public thiefVaults;
    uint public copVaults;
  ...
  modifier setPower(address _Player){
    ...
    function setTheGame() external onlyOwner{
        addBank("Bank of America", 10, 1000);addBank("National Bank", 100, 1000);
        addBank("China Bank", 5, 1000);addBank("Bank Of Egypt", 4, 1000);
        addCop("AngelRubio", 1);addCop("LuisTamayo", 2);
        addThief("Elprofessor", 10);addThief("Rio", 1);addThief("Nairobi", 2);
        addThief("Tokyo", 3);addThief("Denver", 4);addThief("ArturoRoman", 5);
        startTheGame("Berlin");
    }
  ...
  function claimReward() external returns(uint totalAmt){
      require(gameRunning != false, "Game is paused by owners!");
      //require(block.timestamp <= endTime.sub(1 hours), "Please wait one hour after round starts");
      require(playerIsHolder[msg.sender] == true || playerIsCop[msg.sender] == true, "You have no Rewards!");
